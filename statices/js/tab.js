...
layui.define(['element','common'], function(exports) {
	...
	* 删除tab
	* 
	* @param {Object} data: 删除tab所需数据，close_title表示要删除的tab标题，show_title表示删除tab后要显示的tab标题
	*/
	Tab.prototype.tabDel = function (data) {
		var that = this;
		var _config = that.config;
		var showtabIndex = that.exists(data.show_title);
		if(showtabIndex !== -1){
			element.tabChange(ELEM.tabFilter, showtabIndex);
			if(data.showPageRefresh === true){

				_config.elem.find('div.layui-tab-content > div').eq(showtabIndex).children('iframe')[0].contentWindow.location.reload();
			}
		}

		var closetabIndex = that.exists(data.close_title);
		if(closetabIndex !== -1) {
			element.tabDelete(ELEM.tabFilter, closetabIndex).init();
		}
	};
