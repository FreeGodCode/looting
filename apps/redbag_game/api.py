...
from model import (default_values, int_key, status_values, _insert, type_num_values, is_online_values)
...
redbag_game_api_blue = Blueprint('redbag_game_api', __name__, url_prefix='/api/redbag_game')


@redbag_game_api_blue.route('/my_list', methods=['get'])
@login_api_check()
def my_list():
    criteria = {}
    # 支持表格字段里的任何搜索
    ...
    _cur = redbag_game.find(criteria)
    _count = _cur.count()
    if _count % limit:
        num = _count / limit + 1
    else:
    ...
    if _count > (page_num * limit):
        _list = []
        cur_list = _cur.sort([('status', -1), ('is_online', -1), ('sort_id', 1)]).skip(page_num * limit).limit(limit)
        for _obj in cur_list:
            try:
                _obj['_id'] = str(_obj['_id'])
                _obj['status_name'] = status_values.get(_obj.get('status', 0), '-')
                _obj['type_num_name'] = type_num_values.get(_obj.get('type_num', 0), u'老玩法')
                _obj['is_online_name'] = is_online_values.get(_obj.get('is_online', 0), u'未上线')
                _list.append(_obj)
            except Exception, e:
                print e
...
@redbag_game_api_blue.route('/add', methods=['POST'])
@login_api_check()
def add():
...
@redbag_game_api_blue.route('/detail', methods=['get'])
@login_api_check()
def detail():
    access_status = judging_permissions('2_6_3')
    if access_status.get('code') != 200:
    ...
    if not _id or not ObjectId.is_valid(_id):
        return jsonify({'code': 201, 'msg': u'参数错误'})
    _obj = redbag_game.find_one({'_id': ObjectId(_id)})
    if not _obj:
...
@redbag_game_api_blue.route('/update', methods=['post'])
@login_api_check()
def update():
    ...
    if _update:
        try:
            redbag_game.update_one({'_id': ObjectId(_id)}, {'$set': _update})
            return jsonify({'code': 200, 'msg': u'成功'})
        except:
            pass
    else:
...
@redbag_game_api_blue.route('/delete', methods=['delete', 'post'])
@login_api_check()
def delete():
    ...
    redbag_game.remove({'_id': ObjectId(_id)})
    return jsonify({'code': 200, 'msg': u'成功'})
