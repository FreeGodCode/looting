...
planet_img_api_blue = Blueprint('planet_img_api', __name__, url_prefix='/api/planet_img')


@planet_img_api_blue.route('/my_list', methods=['get'])
@login_api_check()
def my_list():
    criteria = {}
    # 支持表格字段里的任何搜索
...
@planet_img_api_blue.route('/add', methods=['POST'])
@login_api_check()
def add():
    data = {key: dict(request.form)[key][0] for key in dict(request.form)}
    if not data:
        data = request.get_json()
    access_status = judging_permissions('2_3_0')
    if access_status.get('code') != 200:
        return jsonify(access_status)
    _file = request.files
...
@planet_img_api_blue.route('/detail', methods=['get'])
@login_api_check()
def detail():
    access_status = judging_permissions('2_3_3')
    if access_status.get('code') != 200:
    ...
    if not _id or not ObjectId.is_valid(_id):
        return jsonify({'code': 201, 'msg': u'参数错误'})
    _obj = planet_img.find_one({'_id': ObjectId(_id)})
    if not _obj:
...
@planet_img_api_blue.route('/update', methods=['post'])
@login_api_check()
def update():
    access_status = judging_permissions('2_3_2')
    if access_status.get('code') != 200:
        return jsonify(access_status)
    _update = {}
    data = {key: dict(request.form)[key][0] for key in dict(request.form)}
    ...
    if _update:
        try:
            planet_img.update_one({'_id': ObjectId(_id)}, {'$set': _update})
            return jsonify({'code': 200, 'msg': u'成功'})
        except:
            pass
    else:
...
@planet_img_api_blue.route('/delete', methods=['delete', 'post'])
@login_api_check()
def delete():
    access_status = judging_permissions('2_3_1')
    if access_status.get('code') != 200:
        return jsonify(access_status)
    data = {key: dict(request.form)[key][0] for key in dict(request.form)}
    if not data:
    ...
    planet_img.remove({'_id': ObjectId(_id)})
    return jsonify({'code': 200, 'msg': u'成功'})
