...
from model import (default_values, int_key, status_values, _insert, open_system_values)
...
@app_wake_task_api_blue.route('/my_list', methods=['get'])
@login_api_check()
def my_list():
    criteria = {}
    # 支持表格字段里的任何搜索
    access_status = judging_permissions('5_1_3')
    ...
    _cur = app_wake_task.find(criteria)
    _count = _cur.count()
    if _count % limit:
        num = _count / limit + 1
    else:
...
@app_wake_task_api_blue.route('/add', methods=['POST'])
@login_api_check()
def add():
    data = {key: dict(request.form)[key][0] for key in dict(request.form)}
    if not data:
        data = request.get_json()
    access_status = judging_permissions('5_1_0')
    if access_status.get('code') != 200:
        return jsonify(access_status)
    _file = request.files
...
@app_wake_task_api_blue.route('/detail', methods=['get'])
@login_api_check()
def detail():
    access_status = judging_permissions('5_1_3')
    if access_status.get('code') != 200:
    ...
    if not _id or not ObjectId.is_valid(_id):
        return jsonify({'code': 201, 'msg': u'参数错误'})
    _obj = app_wake_task.find_one({'_id': ObjectId(_id)})
    if not _obj:
...
@app_wake_task_api_blue.route('/update', methods=['post'])
@login_api_check()
def update():
    access_status = judging_permissions('5_1_2')
    if access_status.get('code') != 200:
        return jsonify(access_status)
    _update = {}
    data = {key: dict(request.form)[key][0] for key in dict(request.form)}
    ...
    if _update:
        try:
            app_wake_task.update_one({'_id': ObjectId(_id)}, {'$set': _update})
            return jsonify({'code': 200, 'msg': u'成功'})
        except:
            pass
    else:
...
@app_wake_task_api_blue.route('/delete', methods=['delete', 'post'])
@login_api_check()
def delete():
    access_status = judging_permissions('5_1_1')
    if access_status.get('code') != 200:
        return jsonify(access_status)
    data = {key: dict(request.form)[key][0] for key in dict(request.form)}
    if not data:
    ...
    app_wake_task.remove({'_id': ObjectId(_id)})
    return jsonify({'code': 200, 'msg': u'成功'})
